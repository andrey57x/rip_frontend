...
export async function getCartInfo(
  ...
  try {
    const response = await axiosInstance.get<{ id: number; reactions_count: number; cart_icon?: string }>(
      "/mass-calculations/mass-calculation-cart-icon",
      { signal }
    );
    return response.data;
  } catch (error) {
...
export async function login(credentials: UserCredentials): Promise<AuthResponse> {
  const response = await axiosInstance.post<AuthResponse>("/users/sign-in", credentials);
  return response.data;
}
...
export async function fetchCalculationById(calculationId: number): Promise<MassCalculationDetails> {
  const response = await axiosInstance.get<MassCalculationDetails>(`/mass-calculations/${calculationId}`);
  return response.data;
}

export async function fetchCalculationsHistory(filters?: HistoryFilters): Promise<MassCalculationInList[]> {
  const response = await axiosInstance.get<MassCalculationInList[]>("/mass-calculations", { params: filters });
  return response.data;
}

export async function removeReactionFromDraft(calculationId: number, reactionId: number): Promise<void> {
  await axiosInstance.delete(`/reaction-calculations/${calculationId}/${reactionId}`);
}
...
export async function moderateCalculation(id: number, status: 'completed' | 'rejected'): Promise<MassCalculationInList> {
    const response = await axiosInstance.put(`/mass-calculations/${id}/moderate`, { status });
    return response.data;
}
...
export async function updateReactionMass(payload: UpdateMassPayload): Promise<void> {
