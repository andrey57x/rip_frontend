...
const CalculationsListPage: React.FC = () => {
  ...
  const handleModerate = (id: number, newStatus: "completed" | "rejected") => {
    dispatch(moderate({ id, status: newStatus }));
  };

  const processedHistory = useMemo(() => {
    const filtered = filters.creator
      ? history.filter((calc) =>
          calc.creator_login
            .toLowerCase()
  ...
  return (
    <>
      ...
      <main className="calc-history-page">
        ...
        {status !== "loading" && processedHistory.length === 0 && (
          ...
          <table className="calc-history-table">
            <thead>
              <tr>
                ...
                <th>Автор</th>
                <th>Действия</th>
                {user?.isModerator && <th>Модерация</th>}
              </tr>
            </thead>
            <tbody>
              {processedHistory.map((calc) => (
                <tr key={calc.id}>
                  <td>#{calc.id}</td>
